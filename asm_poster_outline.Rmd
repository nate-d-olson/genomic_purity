...
single_matches <- ldply(single_matches,.fun = read.csv, stringsAsFactors = F)
...
# Header
## Title   
Development of methods for using the pathoscope sequence analysis software to validate microbial forensics test material purity.
...
## Author Affiliation  
Biosystems and Biomaterials Division, National Insitutute of Standards and Technology   

## Contact  
email nolson@nist.gov

# Abstract
Method validation for microbial forensics requires high purity test materials.  Traditional methods for evaluating test material purity, e.g. polymerase chain reaction (PCR), require prior knowledge of the contaminant identity and have a limited level of detection (LOD).  Whole genome sequencing using next generation sequencing (NGS) technology addresses these limitations by providing a non-target specific approach and a lower LOD due to the large number of reads generated in a sequencing run – ten thousand to over one million reads.  Here we present a novel application of the pathoscope sequence analysis program (http://sourceforge.net/projects/pathoscope/) coupled with an expanded reference dataset to evaluate test material purity and validate material identity using simulated whole genome sequencing datasets.  To evaluate the ability of pathoscope to identify contaminants in test materials, in silico generated datasets for seven organisms (Bacillus anthracis, Escherichia, Francisella tularensis, Pseudomonas aeruginosa , Staphylococcus aureus, and Yersinia pestis) were generated based on the error profile for Illumina data.   First, we used pathoscope to predict material purity when reads were generated from a single organism. Y. pestis, S. aureus, and P. aeruginosa all had greater than 99% specificity for strain level identification.  E. coli, B. anthracis, and F. tularensis had strain level specificity values of 86.2 %, 91.7 %, and 87.2 % respectively.  To determine the method LOD, simulated reads from individual datasets were mixed at known proportions. All contaminant organisms were detected to proportions of at least 1 in 100, and P. aeruginosa and S. aureus were detectable as contaminants in all target organisms down to proportions of 1 in 107.  The approach presented here provides a sample independent, highly sensitive, and specific method for evaluating test material purity.  With the rapid decrease in NGS costs, this approach is a viable alternative to PCR based methods for evaluating test material purity and can be applied in a high throughput manner for screening panels of test materials used to validate biothreat detection technologies.   

# Introduction
Biodetection
Test materials for method validation
Need for pure reference materials

Whole genome sequencing
Large sample size - increased power and lower limit of detection
bioinformatic approach - accurate taxonomic classification
pathoscope

Suspected biothreat events continue to be a pressing issue, with on average six site visits and five phone calls weekly requiring the response of the Federal Bureau of Investigation (FBI)[1].  ASTM E2770-10: Standard Guide for Operational Guidelines for Initial Response to a Suspected Biothreat Agent provides guidance for responding to suspected biothreats and describes the need for method validation for field-based biothreat detection technologies [2].  Critical to method validation for microbial forensics is assessment of a procedure’s ability to obtain reliable results under defined conditions [3].  Appropriate, well-defined, and characterized biological test materials are critical to method validation.  
Association of Analytical Communities (AOAC) International formed the Stakeholder Panel on Agent Detection Assays (SPADA) to define test materials used to validate biothreat detection assays [4].  SPADA defined inclusivity and exclusivity panels to use in PCR-based assay validation for Francisella tularensis, Yersinia pestis, and Bacillus anthracis [5-7].  The F. tularensis validation method includes 9 inclusivity and 10 exclusivity strains, the Y. pestis panel has 16 inclusivity and 17 exclusivity strains, and the B. anthracis has 15 inclusivity and 20 exclusivity strains.  However, before these strains can be used as test materials to validate detection assays, the test materials themselves must be characterized in terms of purity to eliminate false positive or negative results due to test material contamination.  Evaluating test material purity requires assaying for the presence of DNA or cells from contaminants within the test material.  Ideally, only test materials with a well-characterized (and high) purity should be used to validate assays for biothreat detection, as decisions that affect public health and safety will be based on those assays. 
...
# Methods
The bioinformatics approach to measuring test material purity consists of three steps.  First, sequencing reads are generated for the test material using shotgun sequencing.  Next the reads are mapped to a reference database.  For this step individual reads are aligned to all possible locations in the genomes in the reference database.   We generated a more exhaustive database compared to the pathoscope database including all prokaryotic organisms in the Genbank database (http://www.ncbi.nlm.nih.gov/genbank/, accessed 10/18/2013).  Third, the resulting mapping file is processed with pathoscope to assess dataset composition [12].
...
To evaluate the specificity and sensitivity of the approach, simulated single organism as well as contaminated datasets (mixtures of two organisms) were generated.  The organisms used in the study included common biothreat agents and foodborne human pathogens (Table 1).  Simulated datasets were generated using the ART sequencing read simulator.  The ART simulator used error models based on real sequence datasets to generate more realistic datasets.  The datasets were generated using the Illumina error models with paired end 75 and 250 base pair reads and 20 X mean coverage for each of the strains [13].

Simulated whole genome sequencing datasets for `r length(unique(single_matches$input_filename))` organisms from 9 different genus (Table 1) were used to evlauate the specificity of the approach. To evaluate the specificity of the approach, datasets from the individual organisms were mapped to the all-bacterial genomes database using bwa mem and writing all potential matches for ambiguously mapped reads to the mapping file.  Pathoscope requires that all matches for ambiguously mapped reads be written to the output file; it is important to note that not all mapping algorithms have the option to write all potential matches to file.  The resulting mapping file was processed using pathoscope.  The specificity of the individual organisms was evaluated at the strain, species, and genus level.
...
##---###  Need to fix ### issue with last proportion
proportions <- str_c(unique(contam_matches$prop1),unique(contam_matches$prop2), sep = ":")
```

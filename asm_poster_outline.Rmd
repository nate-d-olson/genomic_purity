...
```{r org_names, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r sim-unique-counts, echo=FALSE, results='hide'}
...
```{r filtered-contam, echo=FALSE, results='hide', cache=TRUE}
...
```{r single_counts_df, echo=FALSE, results='hide'}
...
contam_names$org2_tid <- NULL
```

```{r contam_genome_matches, echo=FALSE}
contam_counts <- ddply(contam_filtered_250, 
                       .(org1_tid, org2_tid, match_genus), 
                       summarize, 
                       counts =length(unique(match_tid)))
contam_counts$org2_name <- org_names$ab_name[match(contam_counts$org2_tid,org_names$org_tid)]
```
...
========================================================
# TODO
Check single_matches and contam matches for -1 in org_tids and NA in matches
Check for multiple entries in the single org datasets
make supplemental material table with complete list of single organisms
workout adding taxa names to figures
add org2 and org1 genus
...
## Specificity    
The results from analyzing single organism datasets was analyzed to assess the specificity of the method.  Here specificity is defined as the methods ability of accuractely assign the reads to the appropriate organism.   
```{r single-counts-plot, echo=FALSE, fig.width = 10, fig.height=15}
...
ggplot(single_counts) + 
    ...
    scale_x_continuous(breaks = 1:length(levels(single_counts$variable)), 
...
```
For all organisms the number of unqiue matches decreases with increase in the length of the simulated reads. The number of matches is dependent on the genetic diversity of the genomes within the genus in the database. Genus with greater diversity will have fewer unique hits as fewer reads map incorectly to other genomes.    
#$# Note to self - should I test the impact of genetic diversity on number of unique counts and if so how?
...
Overall distribition of Final.Guess values.  Final.Guess is the proprotion of the total number of aligned reads to the number of a reads assinged to a genome in the reference database. The 90 % of the hits have a Final.Guess values of less than `r quantile(single_matches$Final.Guess, probs= 0.90)`.  For most of the matched only a small fraction of the reads are mapped to the match organisms genome in the reference database.   

```{r final-guess-hist, echo=FALSE, message=FALSE}
qplot(x = Final.Guess, data = single_matches, geom= "bar") + scale_x_log10() + theme_bw() + labs(x = "Estimated Proportions", y = "Abundance")
```

## Sensitivity
The sensitivity of the method was evaluated by on the methods ability to detect the in silico contaminant spiked in at different proportions.  Sensitivity was assessed for 250 bp reads and the genus of the reference organisms the mapped reads were assigned to.  
### species and strain level evaluation of the matches for the 250 bp and all match levels for the 75 bp datasets- the specificity of the method was too low.  Difficult to differentiate the noise from the contaminant signal.

## Need an inital plot showing the number of genus matched for foreach org1 org2 pair or at least each org1
```{r contam_genome_matches_plot, fig.width=10, fig.height=4, echo=FALSE}
ggplot(contam_counts) + 
  geom_raster(aes(x = org2_name, y = match_genus, fill = match_genus), show_guide = F) + 
  facet_grid(~org1_tid, scale = "free_x", labeller = org_tid_labeller) +
  theme_bw() +
...
ggplot(contam_genus_match) + 
  geom_raster(aes(x= as.factor(prop2), y = org2_tid, fill = match_genus)) + 
  facet_grid(org1_tid~., scale = "free_y", labeller = org_tid_labeller) +
  labs(x = "Contaminant Spike Level", 
       y = "Contaminant Organsim", 
       fill = "Contaminant Genus") +
  theme_bw() +
  theme(axis.text.y = element_blank(), strip.text.y = element_text(face="italic"), legend.position = "bottom", legend.direction="horizontal")
```

Even with baseline removal a number of hits to the target organisms genome

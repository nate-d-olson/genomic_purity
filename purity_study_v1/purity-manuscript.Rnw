...
Only a few reads were mapped to a majority of the sequences in the database (Fig. \ref{fig:final-guess-plot}). The estimated proportion of reads in the datasets mapped to a genome in the database 90 \% have an estimated proportion less than \Sexpr{ quantile(single_matches$Final.Guess, probs= 0.90)}.  Most of the reads are mapped to the match organisms genome in the reference database (Fig. \ref{fig:final-guess-exact-plot}). Indicating that this method can be used to support claims regarding the identity of hte test material in addition to the test material purity.   This assumes that the test marerial genome sequence is in the database.  The authors of pathoscope test the algorithm performance on datasets where the test organims genome was not in the database and the algoirthm was able to accuractely identify the nearest genome in the reference database \citep{Francis2013}.  
...
The method limit of detection ranged from contamination of 5 \% to 0.000025 \% of the sample (Fig. \ref{fig:contam-LOD-plot}).  Most (31 out of 42) of test material and contaminant combinations having limit of detection of 0.25 \%.  \textit{F. tularensis} and \textit{B. anthracis} had consistently lower estimated limits of detection when spiked into other test materials.  The lower limit of detection is due to the lower geneic diversity of the two genus and therefore fewer reads in the datasets map uniquely to sequences for the genus.  For the \textit{F. tularensis}, \textit{B. anthracis}, and \textit{Y. pestis} only 36, 153, and 470 reads uniquely mapped to the reference sequences for the simulated datasets.  
